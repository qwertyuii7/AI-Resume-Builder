import { FilePenLineIcon, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, Upload, UploadCloudIcon, XIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  const uploadResume = async (e) => {
    ...
    try {
      ...
      console.log("Sending payload:", { title, resumeText: resumeText?.slice(0, 200) });
      ...
      <div className='max-w-7xl mx-auto px-4 py-8'>
            ...
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <div>
                <label htmlFor="resume-input" className='block text-sm text-slate-700'>Select resume file
                  <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-500 cursor-pointer transition-colors'>
                    {resume ? (
              ...
              <button disabled={isLoading} className='w-full py-2 flex items-center justify-center gap-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors'>
        ...
        {editResumeId && (
          <form onSubmit={editTitle} onClick={() => setEditResumeId('')} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <h2 className='text-xl font-bold mb-4 '>Edit Resume Ttile</h2>
              <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required />
